...
Egl90_can_node::Egl90_can_node(ros::NodeHandle _nh) : _cmdRetries(0)
{
  ...
  _respListener = _can_driver.createMsgListener(
      can::MsgHeader(_can_module_id), can::CommInterface::FrameDelegate(this, &Egl90_can_node::handleFrame_response));
  _errorListener = _can_driver.createMsgListener(
      can::MsgHeader(_can_error_id), can::CommInterface::FrameDelegate(this, &Egl90_can_node::handleFrame_error));
...
void Egl90_can_node::handleFrame_response(const can::Frame& f)
{
    ...
    switch (f.data[1])
    {
      case CMD_INFO:
        switch (f.data[2])
        {
          ...
          case INFO_SEARCH_SINE_VECTOR:
            ROS_DEBUG("The space vector for the sine commutation is being searched. Hereby the phases are being "
                      "applied with 60 procent of the max. current.");
            break;
          case INFO_TIMEOUT:
      ...
      case CMD_MOVE_BLOCKED:
      {
        ...
        if (cmdStatus == RUNNING)
        {
          ...
          ROS_INFO("The gripper finished executing the command CMD_REFERENCE but it did not reach the desired position "
                   "because it was blocked by an object.");
          ROS_INFO("Changing the state of command %s from %s to %s", _cmd_str[CMD_REFERENCE].c_str(),
                   _status_cmd_str[(STATUS_CMD)cmdStatus].c_str(), _status_cmd_str[ERROR].c_str());
        ...
        if (cmdStatus == RUNNING)
        {
          ...
          ROS_INFO("The module finished executing the command MOVE_POS but it did not reach the desired position "
                   "because it was blocked by an object.");
          ROS_INFO("Changing the state of command %s from %s to %s", _cmd_str[MOVE_POS].c_str(),
                   _status_cmd_str[(STATUS_CMD)cmdStatus].c_str(), _status_cmd_str[ERROR].c_str());
        ...
        if (cmdStatus == RUNNING)
        {
          ...
          ROS_INFO("Changing the state of command %s from %s to %s", _cmd_str[MOVE_VEL].c_str(),
                   _status_cmd_str[(STATUS_CMD)cmdStatus].c_str(), _status_cmd_str[OK].c_str());
  ...
  {
    ROS_INFO("Got response %x %x, %s %s", search->first, search->second.second, _cmd_str[(CMD)search->first].c_str(),
             _status_cmd_str[(STATUS_CMD)search->second.second].c_str());
    ...
    else
    {
      switch (search->first)
      {
        case CMD_ACK:
          ...
          {
            ROS_INFO("Changing the state of command %s from %s to %s", _cmd_str[(CMD)search->first].c_str(),
                     _status_cmd_str[(STATUS_CMD)search->second.second].c_str(), _status_cmd_str[OK].c_str());
        ...
        case CMD_REFERENCE:
          ...
          {
            boost::mutex::scoped_lock lock(_mutex);
            ROS_INFO("The module acknowledge the receipt of the command %s with ok.",
                     _cmd_str[(CMD)search->first].c_str());
            ROS_INFO("Changing the state of command %s from %s to %s", _cmd_str[(CMD)search->first].c_str(),
                     _status_cmd_str[(STATUS_CMD)search->second.second].c_str(), _status_cmd_str[RUNNING].c_str());
        ...
        case MOVE_POS:
          if (f.dlc >= 2 && f.data[2] == INFO_VALUE_LIMIT_MAX)
          {
            ...
            ROS_INFO("Changing the state of command %s from %s to %s", _cmd_str[(CMD)search->first].c_str(),
                     _status_cmd_str[(STATUS_CMD)search->second.second].c_str(), _status_cmd_str[ERROR].c_str());
            boost::mutex::scoped_lock lock(_mutex);
            search->second.second = ERROR;
            lock.unlock();
            _cond.notify_all();
          ...
          if (f.dlc > 4)
          {
            ...
            ROS_INFO("The module will execute the command %s in %.2f seconds.", _cmd_str[(CMD)search->first].c_str(),
                     timeEstimation.f);
            ROS_INFO("Changing the state of command %s from %s to %s", _cmd_str[(CMD)search->first].c_str(),
                     _status_cmd_str[(STATUS_CMD)search->second.second].c_str(), _status_cmd_str[RUNNING].c_str());
            boost::mutex::scoped_lock lock(_mutex);
        ...
        case CMD_STOP:
          if (f.dlc >= 3 && f.data[2] == REPLY_OK_1 && f.data[3] == REPLY_OK_2)
          {
            ...
            {
              // the message CMD_MOVE_BLOCKED is a confirmation from the gripper for the command MOVE_POS
              ROS_INFO("The gripper finished executing the command MOVE_POS but it did not reach the desired position "
                       "because it was blocked by an object.");
...
void Egl90_can_node::handleFrame_error(const can::Frame& f)
{
  ...
  {
    ROS_WARN("Received warning msg: %x %x, %s %s", f.data[1], f.data[2], _error_str[(ERROR_CODE)f.data[1]].c_str(),
             _error_str[(ERROR_CODE)f.data[2]].c_str());
  }
  else if (f.data[1] == CMD_ERROR)
  {
    ROS_ERROR("Received error msg: %x %x, %s %s", f.data[1], f.data[2], _error_str[(ERROR_CODE)f.data[1]].c_str(),
              _error_str[(ERROR_CODE)f.data[2]].c_str());
  ...
  switch (f.data[1])
  {
    case CMD_WARNING:
      ...
      {
        case ERROR_SOFT_LOW:
          ROS_WARN("Closed the gripper with no object. Issue a movement command(e.g. CMD_REFERENCE, MOVE_POS, "
                   "MOVE_GRIP) to fix it.");
          break;
        case ERROR_SOFT_HIGH:
          ROS_WARN("Opened the gripper until its maximum width. Issue a movement command(e.g. CMD_REFERENCE, MOVE_POS, "
                   "MOVE_GRIP) to fix it.");
          break;
        default:
    ...
    case CMD_ERROR:
      ...
      switch (f.data[2])
      {
        ...
        case NOT_REFERENCED:
          ...
          break;
        default:
          ROS_ERROR("Received error %x, %s. Acknowledge the error to fix it.", f.data[2],
                    _error_str[(ERROR_CODE)f.data[2]].c_str());
          setState(MOVE_VEL, ERROR, false);
          _cond.notify_all();
          break;
...
bool Egl90_can_node::setState(Egl90_can_node::CMD command, Egl90_can_node::STATUS_CMD status, bool warn)
{
  ...
  {
    ROS_ERROR_COND(warn, "Command %s changes status to %s ,but was not assigned before!", _cmd_str[command].c_str(),
                   _status_cmd_str[status].c_str());
    return false;
  }
  else  // CMD was found
  {
...
bool Egl90_can_node::moveToReferencePos(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
{
  ...
  do
  {
    ...
    do
    {
      hasNoTimeout = _cond.timed_wait(
          lock, timeout);  // if timer expires then hasNoTimeout = false; if _cond.notify_all() then hasNoTimeout = true
      if (hasNoTimeout == false)
      {
        ROS_INFO("The timer has expired. The module received the command CMD_REFERENCE but it did not execute it. Must "
                 "resend it.");
        removeState(CMD_REFERENCE);
...
bool Egl90_can_node::acknowledge(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res)
{
  ...
  do
  {
    ...
    do
    {
      ...
      std::cout << "_shutdownSignal = " << _shutdownSignal << ", isDone = " << cmdDone
                << ", hasNoTimeout = " << hasNoTimeout << std::endl;
...
void Egl90_can_node::gripperCmdAction(const control_msgs::GripperCommandGoalConstPtr& goal, GripperActionServer* as)
{
  ...
  ROS_INFO("got gripper action for target pos %f, current pos %f and max eff %f", target_width, current_width,
...
bool Egl90_can_node::moveGrip(schunk_gripper_egl90::MoveGrip::Request& req,
                              schunk_gripper_egl90::MoveGrip::Response& res)
{
  // move_grip seems not to be available in module 12, this is the alternative implementation using velocity cmd and
  // underlying current control!
  ...
  if (_cmd_map.count(cmd) == 1)
  {
    switch (_cmd_map[cmd].second)
    {
      case ERROR:
      {
        ...
        error_flag = true;
        bool removed = removeState(
            cmd);  // remove the command only if the gripper acknowledged the receipt of the command with an error
        if (removed)
        {
      ...
      default:
        ROS_INFO("In Egl90_can_node::isDone(...): command %s has state %s", _cmd_str[cmd].c_str(),
                 _status_cmd_str[(STATUS_CMD)_cmd_map[cmd].second].c_str());
        break;
    }
  }
  else
  {
    ROS_INFO("The command %s was removed from _cmd_map because it was not executed by the module!",
             _cmd_str[cmd].c_str());
  }
  return isDone;
}
...
void Egl90_can_node::spin()
{
  ...
  std::string nodename = ros::this_node::getName();
  Egl90_can_node::GripperActionServer action_srv_gripper_command(
      _nh, nodename + "/gripper_action",
      boost::bind(&Egl90_can_node::gripperCmdAction, this, _1, &action_srv_gripper_command), false);
